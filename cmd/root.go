...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.ali-ecs-tag-update.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".ali-ecs-tag-update" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".ali-ecs-tag-update")
	}
