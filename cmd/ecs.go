...
var ecsCmdFlags = alicloud.QueryFlags{}
...
for example:
  ali-ecs-tag-update ecs --regname 'worker-k8s.*' --logfile /tmp/ecs_update.log --loglevel debug
  ali-ecs-tag-update ecs --regname 'worker-k8s.*' --notagk Environment --cron '* * * * * *'`,
	Run: func(cmd *cobra.Command, args []string) {
		pm := monitor.NewPrometheusMonitor()
		var c *cron.Cron
...
func queryECStag(job joblock.JobLock, aliClient *alicloud.AliClient, queryFlags alicloud.QueryFlags, pm *monitor.PrometheusMonitor, instanceList map[string]ecs.Instance) error {
...
func init() {
	...
	f.VarP(&ecsCmdFlags.RegName, "regname", "", "filter by ecs instance name with regular expression  example: ecs.* (can specify multiple, will use or operator)")
