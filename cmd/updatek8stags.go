...
var updateK8sTagsCmdFlags = alicloud.QueryFlags{}

// updateK8sTagsCmd represents the ecs command
var updateK8sTagsCmd = &cobra.Command{
	Use:   "updatek8stags",
	Short: "ECS tag update for k8s",
	Long: `This tool will update ecs tag for k8s worker running on Alicloud. 

for example:
  ali-ecs-tag-update updatek8stags --logfile /tmp/ecs_update.log --loglevel debug
  ali-ecs-tag-update updatek8stags --cron '0 * * * * *'`,
...
func addk8sTags(job joblock.JobLock, aliClient *alicloud.AliClient, pm *monitor.PrometheusMonitor, instanceList map[string]ecs.Instance) error {
...
func init() {
	...
	updateK8sTagsCmdFlags.RegName = append(updateK8sTagsCmdFlags.RegName, "worker-k8s.*")
